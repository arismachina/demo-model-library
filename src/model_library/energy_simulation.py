...
Simulates cell energy capacity using PyBaMM with cell design parameters and operating conditions.
"""
...
def simulate_energy(
    ...
    """
    Simulate cell energy capacity using PyBaMM.
    ...
    Args:
        ...
        simulation_config: Simulation configuration dictionary containing:
            ...
            - c_rates: List of C-rates for discharge (default: [0.2, 0.5, 1.0])
    ...
    Returns:
        List of dictionaries, one per C-rate, each containing:
            - c_rate: The C-rate for this result
            - discharge_capacity_Ah: Discharge capacity [Ah]
            - discharge_energy_Wh: Discharge energy [Wh]
            - average_voltage_V: Average discharge voltage [V]
            - max_temperature_C: Maximum temperature during discharge [°C]
            - duration_min: Discharge duration [min]
            - time_s: Array of time points [s]
            - voltage_V: Array of voltages during discharge [V]
            - current_A: Array of currents during discharge [A]
            - energy_Wh: Array of cumulative energy [Wh]
            - config: Configuration used

    Example:
        >>> config = {"temperature_K": 298.15, "c_rates": [0.2, 0.5, 1.0]}
        >>> results = simulate_energy(cell_design_manifest, config)
        >>> for r in results:
        ...     print(f"{r['c_rate']}C: {r['discharge_energy_Wh']:.2f} Wh")
    """
    ...
    operating_conditions = {
        ...
        "Current function [A]": simulation_config.get("c_rate", 0.2)
    ...
    print("\n" + "=" * 80)
    print("ENERGY SIMULATION")
    print("=" * 80)

    # Get C-rates list (support both single c_rate and c_rates list)
    c_rates = simulation_config.get("c_rates", None)
    if c_rates is None:
        # Fallback to single c_rate for backwards compatibility
        c_rates = [simulation_config.get("c_rate", 0.2)]

    lower_voltage = simulation_config.get("lower_voltage_cutoff", cell_design.lower_voltage_cutoff.value)
    initial_soc = simulation_config.get("initial_soc", 1.0)
    ...
    print(f"  Lower voltage cutoff: {lower_voltage} V")
    ...
    for c_rate in c_rates:
        ...
        # Create energy discharge experiment: full discharge from 100% SOC
        # Calculate max discharge time based on C-rate (with margin)
        max_discharge_time_s = int(3600 / c_rate * 1.2)  # 20% margin

        experiment = pybamm.Experiment(
            [
                ("Rest for 1 seconds"),
                (
                    f"Discharge at {c_rate}C for {max_discharge_time_s} seconds or until {lower_voltage} V",
                ),
                ("Rest for 60 seconds",),
            ],
            period="1 second",
        ...
        try:
            ...
            # Extract discharge cycle data (cycle 1 is the discharge)
            discharge_cycle = solution.cycles[1]

            # Get time, voltage, current, and energy arrays
            time_s = discharge_cycle["Time [s]"].entries
            voltage_V = discharge_cycle["Terminal voltage [V]"].entries
            current_A = discharge_cycle["Current [A]"].entries
            energy_Wh = discharge_cycle["Discharge energy [W.h]"].entries
            capacity_Ah = discharge_cycle["Discharge capacity [A.h]"].entries
            temperature_K = discharge_cycle["Volume-averaged cell temperature [K]"].entries
            ...
            # Build results dictionary for this C-rate
            result = {
                "c_rate": c_rate,
            ...
            print(f"    Capacity: {discharge_capacity_Ah:.2f} Ah")
            print(f"    Energy: {discharge_energy_Wh:.2f} Wh")
            print(f"    Avg Voltage: {average_voltage_V:.3f} V")
            print(f"    Duration: {duration_min:.1f} min")
            print(f"    Max Temp: {max_temperature_C:.1f}°C")

        except pybamm.SolverError as e:
            print(f"  ✗ {c_rate}C failed: {str(e)[:60]}")
            all_results.append({
                "c_rate": c_rate,
    ...
    print("=" * 80)
    print(f"{'C-rate':<10} {'Capacity [Ah]':<15} {'Energy [Wh]':<15} {'Avg V [V]':<12} {'Duration [min]':<15} {'Max Temp [°C]':<15}")
    print("-" * 80)
    for r in all_results:
        if r["success"]:
            print(f"{r['c_rate']:<10.1f} {r['discharge_capacity_Ah']:<15.2f} {r['discharge_energy_Wh']:<15.2f} {r['average_voltage_V']:<12.3f} {r['duration_min']:<15.1f} {r['max_temperature_C']:<15.1f}")
        else:
            print(f"{r['c_rate']:<10.1f} {'FAILED':<15} {'-':<15} {'-':<12} {'-':<15} {'-':<15}")
    print("=" * 80)
