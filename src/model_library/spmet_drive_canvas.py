...
# Pack configuration
cells_series = int(componentInputs.get("cells_in_series", 6))
cells_parallel = int(componentInputs.get("cells_in_parallel", 4))

# Vehicle parameters
drone_weight = componentInputs.get("vehicle_weight_kg", 5.0)
drag_coeff = componentInputs.get("drag_coefficient", 1.0)
frontal_area = componentInputs.get("frontal_area_m2", 0.04)
drivetrain_eff = componentInputs.get("drivetrain_efficiency", 0.75)
...
def run_drive_cycle(cell_design, simulation_config):
    ...
    if cathode_material == "LFP" or "LFP" in cathode_material:
...
for design_id, d in cell_design_data.items():
    ...
    cell_design = {
        ...
        "upper_voltage_cutoff": {
            "value": d["cell_design.upper_voltage_cutoff.value"]
        },
        "lower_voltage_cutoff": {
            "value": d["cell_design.lower_voltage_cutoff.value"]
        },
