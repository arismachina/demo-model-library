...
Simulates constant power discharge using PyBaMM with cell design parameters and operating conditions.
"""
...
def simulate_power(
    ...
    """
    Simulate constant power discharge at multiple power levels using PyBaMM.
    ...
    Args:
        ...
        simulation_config: Simulation configuration dictionary containing:
            ...
            - power_levels_W: List of power levels to test [W] (default: [100, 200, 300, 400, 500])
    ...
    Returns:
        List of dictionaries, one per power level, each containing:
            - power_W: The power level for this result
            - discharge_energy_Wh: Discharge energy [Wh]
            - discharge_time_s: Discharge duration [s]
    ...
    Example:
        >>> config = {"temperature_K": 298.15, "power_levels_W": [100, 200, 300, 400, 500]}
        >>> results = simulate_power(cell_design_manifest, config)
        >>> for r in results:
        ...     print(f"{r['power_W']}W: {r['discharge_energy_Wh']:.2f} Wh")
    """
    ...
    operating_conditions = {
        ...
        "Current function [A]": kpis.nominal_capacity.value,  # 1C as default
    ...
    # Get power levels list
    power_levels_W = simulation_config.get("power_levels_W", [100, 200, 300, 400, 500])
    ...
    print(f"  Power levels: {power_levels_W} W")
    ...
    # Results list for all power levels
    all_results = []

    print("\n" + "-" * 80)
    for power_W in power_levels_W:
        print(f"\nRunning {power_W}W discharge simulation...")

        # Create power discharge experiment
        # Estimate max time based on energy capacity and power
        nominal_energy_Wh = target_capacity_Ah * 3.2  # Approximate for LFP
        max_discharge_time_s = int(nominal_energy_Wh / power_W * 3600 * 1.2)  # 20% margin

        experiment = pybamm.Experiment(
            [
                ("Rest for 1 seconds"),
                (
                    f"Discharge at {power_W}W for {max_discharge_time_s} seconds or until {lower_voltage} V",
                ),
                ("Rest for 60 seconds",),
            ],
            period="1 second",
        ...
        try:
            ...
            # Build results dictionary for this power level
            result = {
                "power_W": power_W,
            ...
            print(f"  ✓ {power_W}W completed:")
            print(f"    Energy: {discharge_energy_Wh:.2f} Wh")
            print(f"    Duration: {discharge_time_s/60:.1f} min")
            print(f"    Max Temp: {max_temperature_C:.1f}°C")

        except pybamm.SolverError as e:
            print(f"  ✗ {power_W}W failed: {str(e)[:60]}")
            all_results.append({
                "power_W": power_W,
                "discharge_energy_Wh": 0,
                "discharge_time_s": 0,
                "discharge_time_min": 0,
                "max_temperature_C": 0,
                "time_s": np.array([]),
                "voltage_V": np.array([]),
                "current_A": np.array([]),
    ...
    print("=" * 80)
    print(f"{'Power [W]':<12} {'Energy [Wh]':<15} {'Duration [min]':<15} {'Max Temp [°C]':<15}")
    print("-" * 80)
    for r in all_results:
        if r["success"]:
            print(f"{r['power_W']:<12} {r['discharge_energy_Wh']:<15.2f} {r['discharge_time_min']:<15.1f} {r['max_temperature_C']:<15.1f}")
        else:
            print(f"{r['power_W']:<12} {'FAILED':<15} {'-':<15} {'-':<15}")
    print("=" * 80)
