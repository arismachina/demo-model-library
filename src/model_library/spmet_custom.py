...
def run_spmet(
    ...
    Example:
        ...
        ...     ...
        ... }
        >>> results = run_spmet_manifest(cell_design_manifest, config)
    """
    ...
    # Normalize input: support both full manifest and cell_design sub-dict
    if "cell_design" in cell_design:
        cell_design_dict = cell_design["cell_design"]
    else:
        cell_design_dict = cell_design

    # Build PyBaMM parameters from manifest
    default_params = _build_parameters(cell_design_dict, simulation_config)

    # Convert to object for dot notation access
    cell_design = dict2obj(cell_design_dict)
...
def _build_parameters(
    ...
    # Keep dict reference for material extraction
    cell_design_dict = cell_design
    # Convert to object for dot notation access
    cell_design = dict2obj(cell_design_dict)

    # Get material data from cell_design (loaded by notebook)
    pos_material_data = cell_design_dict["positive_electrode"]["material"]
    neg_material_data = cell_design_dict["negative_electrode"]["material"]
    ...
    # Cell parameters
    cell_params = {
        "Open-circuit voltage at 0% SOC [V]": cell_design.lower_voltage_cutoff.value,
        "Open-circuit voltage at 100% SOC [V]": cell_design.upper_voltage_cutoff.value,
        "Nominal cell capacity [A.h]": cell_design.nominal_capacity.value,
        "Number of cells connected in series to make a battery": 1.0,
        "Current function [A]": cell_design.nominal_capacity.value,  # Default to 1C
        "Typical current [A]": cell_design.nominal_capacity.value,  # Default to 1C
    }
    ...
    separator_params = {
        ...
        "Separator density [kg.m-3]": separator.material.physical_properties.density.value
        * 1000,
        "Separator specific heat capacity [J.kg-1.K-1]": separator.material.thermal_properties.specific_heat_capacity.value,
        "Separator Bruggeman coefficient (electrolyte)": separator.material.thermal_properties.bruggeman_coefficient.value,
        "Separator thermal conductivity [W.m-1.K-1]": separator.material.thermal_properties.thermal_conductivity.value,
    }
    ...
    electrolyte_params = {
        ...
        * 1000,
        "Typical electrolyte concentration [mol.m-3]": electrolyte.composition.nominal_concentration.value
        * 1000,
...
def _run_experiment(
    ...
    - Standard experiment strings (e.g., "Discharge at 1C for 1 hour")
    - Custom termination conditions (anode_potential_threshold_V, jelly_roll_temperature_threshold_K)
    - Fast charge with anode potential riding (ride_anode_potential=True)

    Fast charge config options:
        - ride_anode_potential: bool - Enable anode potential riding after CC phase
        - anode_potential_threshold_V: float - Anode potential threshold (default: 0.02V)
        - max_charge_time_s: float - Maximum total charge time (default: 3600s)
        - cv_termination_c_rate: float - CV phase termination C-rate (default: 0.05 = C/20)
    """
    import re
    ...
    if simulation_config.get("ride_anode_potential"):
        ...
        c_rate_match = re.search(r"at\s+([\d.]+)C", experiment_str)
        ...
        base_exp_str = re.sub(r"\s+or until\s+[\d.]+\s*V", "", experiment_str)

        # Determine voltage limit based on direction
        if "charge" in experiment_str.lower():
